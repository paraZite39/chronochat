...
```bash
chronochat-backend$ sam build --use-container
```
...
```bash
chronochat-backend$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
chronochat-backend$ sam local start-api
chronochat-backend$ curl http://localhost:3000/
```
...
```bash
chronochat-backend$ sam logs -n HelloWorldFunction --stack-name chronochat-backend --tail
```
...
```bash
chronochat-backend$ pip install -r tests/requirements.txt --user
# unit test
chronochat-backend$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
chronochat-backend$ AWS_SAM_STACK_NAME="chronochat-backend" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "chronochat-backend"
```
